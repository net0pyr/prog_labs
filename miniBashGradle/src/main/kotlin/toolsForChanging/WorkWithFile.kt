...
class WorkWithFile() {
    ...
    fun getFromFile(id: Long): SpaceMarine? {
        ...
        if(numberStartLine >= fileArray.size) {
            return null
        }
        var json = fileArray[numberStartLine]
        val len = json.length - 1
        for (i in len downTo 0) {
            json = json.dropLast(1);
            if(fileArray[numberStartLine][i]==',') {
                break
            }
        }
        val spaceMarine = Json.decodeFromString<SpaceMarine>(json)
        return spaceMarine*/
    }
}